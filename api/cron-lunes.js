// api/cron-lunes.js — Cron job que corre todos los lunes a las 21hs Uruguay (00:00 UTC martes)
// Configurado en vercel.json como cron: "0 0 * * 2"

export default async function handler(req, res) {
  // Verificar que es llamada del cron de Vercel (seguridad)
  ...
  try {
    // 1. Obtener datos actuales de ACG
    const preciosRes = await fetch("https://lote21-proxy.vercel.app/api/precios", {
    ...
    function color(actual, ant) {
      ...
      if (a > b) return "#22b14c";
      if (a < b) return "#e63946";
      return "#f4a261";
    }

    // URL del generador con parámetros precargados
    const params = new URLSearchParams({
      nov: novillo, vac: vaca, vaq: vaquillona,
      anov: novilloAnt, avac: vacaAnt, avaq: vaquillonaAnt,
      bov: d.faena?.bovinos || "", ovi: d.faena?.ovinos || "",
      abov: d.faena?.bovinosAnt || "", aovi: d.faena?.ovinosAnt || "",
      sem
    });
    const generadorUrl = `https://lote21-proxy.vercel.app/api/generador?${params.toString()}`;

    // 2. Armar HTML del email
    const emailHtml = `
<!DOCTYPE html>
<html lang="es">
      ...
      <table width="600" bgcolor="#ffffff" cellpadding="0" cellspacing="0" style="border-radius:10px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.1);">
        ...
        <tr><td style="padding:28px 32px 16px;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              ${["NOVILLO","VACA","VAQUILLONA"].map((label, i) => {
                const vals = [novillo, vaca, vaquillona];
                const ants = [novilloAnt, vacaAnt, vaquillonaAnt];
                const v = vals[i], a = ants[i];
                const f = flecha(v, a), c = color(v, a);
                return `
                <td width="33%" style="text-align:center;padding:0 8px;">
                  <div style="background:#1e3c6e;border-radius:8px;padding:14px 8px;">
                    ...
                    <p style="margin:0;color:white;font-size:32px;font-weight:300;">${v.replace(".",",")} <span style="color:${c};font-size:20px;">${f}</span></p>
              ...
              ${[["BOVINOS", bovinos, bovinosAnt], ["OVINOS", ovinos, ovinosAnt]].map(([label, v, a]) => `
                <td width="50%" style="text-align:center;padding:0 8px;">
                  <div style="background:#f0f4f8;border-radius:8px;padding:14px 8px;">
                    ...
                    <p style="margin:0;color:#1a3a5c;font-size:28px;font-weight:300;">${v}</p>
        ...
        <tr><td style="padding:0 32px 32px;text-align:center;">
          <a href="https://lote21-proxy.vercel.app/generar" 
          ...
          <p style="margin:12px 0 0;color:#aaa;font-size:12px;">Hacé click para abrir el generador con los datos ya cargados</p>
        </td></tr>
